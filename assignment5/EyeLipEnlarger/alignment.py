...
class Alignment:
    ...
    def big_filter(self, image, type_filter):
        ...
        result_big = cv2.resize(result, (0,0), fx=2, fy=2, interpolation=cv2.INTER_NEAREST)
        mask_big = cv2.resize(mask, (0,0), fx=2, fy=2, interpolation=cv2.INTER_NEAREST)
        return result_big, mask_big
